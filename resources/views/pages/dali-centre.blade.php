    ...
    <section class="dc-band" {!! cms_section_attr('feature') !!}>
      <div class="wrap dc-diagram reveal">
        <div class="dc-diagram-copy">
          <h2{!! cms_style($meta, 'feature.heading') !!}>{!! accent_html($meta->get('feature.heading')) !!}</h2>
          @if (trim($meta->get('feature.lead')) !== '')
            <p class="dc-diagram-lead"{!! cms_style($meta, 'feature.lead') !!}>{!! accent_html($meta->get('feature.lead')) !!}</p>
          @endif
            ...
            <ol class="dc-diagram-points">
              @foreach ($diagramItems as $item)
                <li>
                  <span class="dc-num">{{ str_pad((string) $loop->iteration, 2, '0', STR_PAD_LEFT) }}</span>
                  <div>
                    <h3{!! cms_style($meta, 'feature.item.title', $loop->index) !!}>{!! accent_html($item['title'] ?? '') !!}</h3>
                    <p{!! cms_style($meta, 'feature.item.body', $loop->index) !!}>{!! accent_html($item['body'] ?? '') !!}</p>
                  </div>
                </li>
              @endforeach
            </ol>
          ...
          <tbody>
            @foreach ($hardwareRows as $row)
              ...
              <tr>
                <td>
                  @if ($preview !== '')
                    <span class="dc-product" data-preview="{{ $preview }}" tabindex="0"{!! cms_style($meta, 'hardware.row.product', $loop->index) !!}>{!! accent_html($row['product'] ?? '') !!}</span>
                  @else
