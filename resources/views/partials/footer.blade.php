@php
    $description = $footerMeta->get(
        'footer.description',
        0,
        'We design, engineer, manufacture, assemble, and test our products in-house, offering custom powder coating to deliver fully tailored lighting solutions',
    );
    ...
    $productLinks = collect($footerMeta->group('footer.products.link'))->filter(
        fn(array $item): bool => trim($item['label'] ?? '') !== '',
    );
    $companyLinks = collect($footerMeta->group('footer.company.link'))->filter(
        fn(array $item): bool => trim($item['label'] ?? '') !== '',
    );
    $contactLinks = collect($footerMeta->group('footer.contact.link'))->filter(
        fn(array $item): bool => trim($item['label'] ?? '') !== '',
    );
@endphp
<footer>
    <div class="wrap">
        ...
        <div class="copy">
            ...
            <div class="copy-links">
                ...
                <a href="{{ url('/modern-slavery') }}">Modern Slavery Statement</a>
            </div>
        </div>
    </div>
</footer>
