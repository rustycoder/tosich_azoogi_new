...
- Sticky `#cards` stack (home values, AI Lighting insights, audience) uses `--card-height: min(48svh, 400px)` (`min(56svh, 400px)` under 720px). Do not raise it on Home or AI Lighting — tall cards float short copy in empty space. Align `.card__content>div` with padding and `align-content: center`, not `place-self: center` + `width: 80%`.
