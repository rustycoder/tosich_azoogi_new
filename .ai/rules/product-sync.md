...
- Persist category `Featured Image`, `Descriptions`, and `Icon` onto `product_categories` (`featured_image`, `description`, `icon`). Home catalogue and `/products` Our Range cards use only the stored description — never hardcoded copy, product descriptions, or a product count.
